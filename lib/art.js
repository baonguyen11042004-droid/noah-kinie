...
function artSvg(p) {
  ...
  const scene = (x, y, w, hgt, r = 6, id = 'a') => `
    ...
    <g clip-path="url(#${id})">
      ...
      <circle cx="${x + w * 0.7}" cy="${y + hgt * 0.32}" r="${w * 0.13}" fill="#fff8ee" opacity=".9"/>
  ...
  let body;
  if (p.category === 'tron') {
    body = `<circle cx="200" cy="200" r="130" fill="#fffaf3" stroke="#f0d9cc" stroke-width="3"/>
      <clipPath id="c"><circle cx="200" cy="200" r="112"/></clipPath>
      <g clip-path="url(#c)"><rect x="80" y="80" width="240" height="240" fill="url(#sky)"/>
      <circle cx="255" cy="150" r="30" fill="#fff8ee" opacity=".9"/>
    ...
    body = `<rect x="40" y="40" width="320" height="320" rx="16" fill="#fffaf3" stroke="#f0d9cc" stroke-width="3"/>${cells.join('')}`;
    ...
    body = `<rect x="60" y="150" width="280" height="190" rx="14" fill="hsl(${h} 45% 82%)"/>
      ...
      <rect x="188" y="120" width="24" height="220" fill="#fffaf3" opacity=".9"/>
      <path d="M200 120c-40-60-90-30-60 0zM200 120c40-60 90-30 60 0z" fill="#fffaf3"/>
      <g transform="rotate(-8 130 90)">${scene(95, 40, 70, 60, 4, 'g1')}</g>`;
  } else {
    body = `<g transform="rotate(-4 200 200)"><rect x="70" y="50" width="260" height="300" rx="10" fill="#fffaf3" stroke="#f0d9cc" stroke-width="3"/>
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400">
    ...
    <radialGradient id="bg"><stop offset="0" stop-color="#fff7ef"/><stop offset="1" stop-color="#fbe4dc"/></radialGradient></defs>
